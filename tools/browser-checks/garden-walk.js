async browserPage => {
 ...
 try{
  ...
  await page.locator('[data-garden-link]').click();await ready(page);
  ...
  for(let i=0;i<6;i++){await page.locator('#mapButton').click();await page.locator(`[data-place="${i}"]`).click();assert(await page.locator(`[data-place="${i}"]`).getAttribute('aria-current')==='true',`Map reaches quiet spot ${i+1}`);}
  ...
  await page.locator('#soundButton').click();assert(await page.locator('#soundButton').getAttribute('aria-pressed')==='true','Optional sound starts');await page.locator('#soundButton').click();
  ...
  await page.locator('#nextButton').click();assert(await page.locator('#placeName').textContent()==='The evening border','Still mode retains instant navigation');
