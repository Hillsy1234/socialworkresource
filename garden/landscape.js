...
export function createLandscape(scene,renderer,mobile){
 ...
 for(let i=0;i<45;i++){const x=rand(-34,34),z=rand(-34,34);if(nearestRoute(x,z).distance<3||inPond(x,z,1))continue;const s=rand(.4,1.2);setRock(x,z,s);obstacles.push({x,z,r:s*.6});}rocks.count=rockCount;rocks.castShadow=true;rocks.receiveShadow=true;scene.add(rocks);projectileColliders.push(rocks);
 ...
 function setDaylight(value){
  daylight=T.MathUtils.clamp(value,0,2);currentTheme=daylight>1.6?'night':daylight>.45?'sunset':'day';
