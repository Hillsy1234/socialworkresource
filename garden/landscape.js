import {createZipline} from './zipline.js';
import {zipClearing,landingClearing} from './zipline-state.mjs';
...
export function createLandscape(scene,renderer,mobile){
 ...
 for(let i=0;i<45;i++){const x=rand(-34,34),z=rand(-34,34);if(landingClearing(x,z,1)||lookoutClearing(x,z,1)||nearestRoute(x,z).distance<3||inPond(x,z,1))continue;const s=rand(.4,1.2);setRock(x,z,s);obstacles.push({x,z,r:s*.6});}rocks.count=rockCount;rocks.castShadow=true;rocks.receiveShadow=true;scene.add(rocks);projectileColliders.push(rocks);
 ...
 for(let i=0;i<bladeCount*2&&blades<bladeCount;i++){const x=rand(-64,40),z=rand(-53,40),near=nearestRoute(x,z);if(landingClearing(x,z)||lookoutClearing(x,z)||near.distance<1.7||inPond(x,z,.25))continue;const scale=rand(.27,.83);dummy.position.set(x,terrainHeight(x,z),z);dummy.rotation.set(0,rand(0,6),rand(-.18,.18));dummy.scale.set(scale,scale*rand(.65,1.3),scale);dummy.updateMatrix();grass.setMatrixAt(blades,dummy.matrix);grass.setColorAt(blades,new T.Color().setHSL(rand(.19,.25),rand(.25,.45),rand(.48,.75)));blades++;}grass.count=blades;grass.receiveShadow=true;scene.add(grass);
 ...
 const lookout=createLookout(scene,wood,metal),zipline=createZipline(scene,wood,metal,renderer);obstacles.push(...zipline.obstacles);
 ...
 let lastRipple=0;
 function update(time,dt,camera,still,opened,waveDt=dt){zipline.updateDetail(camera);wind.value=time;wildlife.update(time,still,season,weather.rain,currentTheme,camera);waves.update(waveDt,still);pondLife.update(time,season,daylight,waves.height);atmosphere.update(time,camera,weather,currentTheme);sun.target.position.set(camera.position.x,0,camera.position.z);sun.position.copy(sun.target.position).addScaledVector(sunDirection,50);if(currentTheme==='night')sun.position.y=25;sun.target.updateMatrixWorld();
 ...
 return {lookout,zipline,projectileColliders,pondWaves:waves,obstacles,benches,shelter,birdCalls:wildlife.calls,streamPosition:pondLife.streamPosition,setSeason,setTheme,setDaylight,setWeather,update,interact,water,hitTargets:[surface,...flowers.map(f=>f.g)]};
}
