...
export const route=new CatmullRomCurve3([[0,30],[-5,20],[-12,12],[-14,1],[-10,-7],[-3,-4],[5,-4],[13,-4],[20,-11],[15,-23],[2,-29],[-12,-24],[-22,-12],[-24,8],[-14,24]].map(([x,z])=>new Vector3(x,0,z)),true,'centripetal');
...
export function canWalk(x,z,obstacles=[]){if(!Number.isFinite(x)||!Number.isFinite(z)||Math.hypot(x,z)>39)return false;const near=nearestRoute(x,z);if(inPond(x,z,.25)&&near.distance>1.08)return false;return !obstacles.some(o=>(x-o.x)**2+(z-o.z)**2<(o.r+.28)**2);}
export function cleanWalkSave(raw){const v=raw&&typeof raw==='object'?raw:{};return {weather:cleanWeather(v.weather),weatherAuto:v.weatherAuto===true,theme:['day','sunset','night'].includes(v.theme)?v.theme:'sunset',still:v.still===true,pace:[.8,1.3,1.8].includes(v.pace)?v.pace:1.3,opened:Array.isArray(v.opened)?[...new Set(v.opened.filter(n=>Number.isInteger(n)&&n>=0&&n<6))]:[]};}
