...
function stopWalking(){pebbles?.cancelAim();breakRemaining=0;audio.resetMotion();guided=false;keys.clear();touchKeys.clear();drag=null;updateControls();}
function updateControls(){if(!started)return;pebbles?.refresh(started&&!still&&!failed);$('guideButton').setAttribute('aria-pressed',String(guided));$('guideButton').textContent=guided?'Pause the walk  Ⅱ':'Follow the path  →';$('pauseButton').disabled=!guided&&!keys.size&&!touchKeys.size;$('guideButton').disabled=still;$('breakButton').disabled=still;$('breakButton').textContent=breakRemaining>0?'End garden break':'2-minute break';$('sitButton').textContent=sitting?'Stand up':'Sit a while';$('sitButton').setAttribute('aria-pressed',String(sitting));$('walkState').textContent=still?'Still view · Choose a place on the map':sitting?'A moment to sit · Drag to look around':guided?(breakRemaining>0?'Your two-minute break · A bench awaits':'Following the path · Look around or pause'):coarse.matches?'Hold arrows to walk · Drag the view to look':'Drag to look · W A S D / arrows · Q / E to turn';$('touchControls').hidden=!started||!coarse.matches||still||sitting;$('controlHint').textContent=still?'Still mode · Nine places to pause.':guided?'Escape or Pause stops the walk.':'Walk at your own pace. No scores. No rush.';}
...
function init(){
 ...
 function positionAt(t){pebbles?.reset();const p=route.getPointAt(t);camera.position.set(p.x,groundHeight(p.x,p.z)+1.67,p.z);yaw=headingAt(t);pitch=-.015;camera.rotation.set(pitch,yaw,0);walkT=t;dirty=true;}
 positionAt(0);
 ...
 function setStill(value){pebbles?.reset();still=value;saved.still=still;stopWalking();$('motionButton').setAttribute('aria-pressed',String(still));$('reticle').hidden=still||!started;dirty=true;save();updateControls();weatherUi();}
 ...
 function acceptsMovement(e){return started&&!still&&!sitting&&!failed&&!$('helpDialog').open&&!$('soundDialog').open&&!$('breathingDialog').open&&!$('visitDialog').open&&!$('readingDialog').open&&$('mapPanel').hidden&&$('weatherPanel').hidden&&!['INPUT','SELECT','TEXTAREA'].includes(e.target?.tagName);}
 ...
 function frame(now){
  ...
  const near=nearestRoute(camera.position.x,camera.position.z),surface=inPond(camera.position.x,camera.position.z,1.55)&&near.distance<1.65?'wood':near.distance<=1.65?(camera.position.x < -30?'grass':'gravel'):'grass';
 ...
 visit=visitControls({saved,save,announce,stopWalking,
  isGuided:()=>guided,prepareQuiet(){pebbles?.cancelAim();keys.clear();touchKeys.clear();drag=null;if(!guided)audio.resetMotion();updateControls();},
