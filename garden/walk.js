...
let visit,pebbles,towerWalk=null;
...
function stopWalking(){towerWalk=null;pebbles?.cancelAim();breakRemaining=0;audio.resetMotion();guided=false;keys.clear();touchKeys.clear();drag=null;updateControls();}
function updateControls(){if(!started)return;pebbles?.refresh(started&&!still&&!failed);$('guideButton').setAttribute('aria-pressed',String(guided));$('guideButton').textContent=guided?'Pause the walk  Ⅱ':'Follow the path  →';$('pauseButton').disabled=!guided&&!towerWalk&&!keys.size&&!touchKeys.size;$('guideButton').disabled=still;$('breakButton').disabled=still;$('breakButton').textContent=breakRemaining>0?'End garden break':'2-minute break';$('sitButton').textContent=sitting?'Stand up':'Sit a while';$('sitButton').setAttribute('aria-pressed',String(sitting));$('walkState').textContent=still?'Still view · Choose a place on the map':sitting?'A moment to sit · Drag to look around':towerWalk?(towerWalk.down?'Following the stairs down · Pause to look around':'Climbing the lookout · Pause to look around'):guided?(breakRemaining>0?'Your two-minute break · A bench awaits':'Following the path · Look around or pause'):coarse.matches?'Hold arrows to walk · Drag the view to look':'Drag to look · W A S D / arrows · Q / E to turn';$('touchControls').hidden=!started||!coarse.matches||still||sitting;$('quietWalk').hidden=!started||still||sitting||failed;$('controlHint').textContent=still?'Still mode · Ten places to pause.':(guided||towerWalk)?'Escape or Pause stops the walk.':'Walk at your own pace. No scores. No rush.';}
...
function init(){
 ...
 function towerView(){stopWalking();sitting=false;camera.position.set(LOOKOUT_TOP.x,LOOKOUT_TOP.y+1.67,LOOKOUT_TOP.z);yaw=Math.atan2(camera.position.x+45,camera.position.z-18);pitch=-.12;dirty=true;updateLocation();updateControls();announce('The woodland lookout. Drag to enjoy the view, or choose Go back down.');}
...
if(guided){const shortBreak=breakRemaining>0,step=shortBreak?breakSpeed*Math.min(elapsed,breakRemaining):saved.pace*dt;if(shortBreak)breakRemaining=Math.max(0,breakRemaining-elapsed);walkT=(walkT+step/ROUTE_LENGTH)%1;guideDistance+=step;const p=route.getPointAt(walkT);camera.position.set(p.x,groundHeight(p.x,p.z)+1.67,p.z);const target=headingAt(walkT)+guideLook,angle=Math.atan2(Math.sin(target-yaw),Math.cos(target-yaw));yaw+=angle*Math.min(1,dt*3);changed=true;if(shortBreak&&breakRemaining===0){arrive(4);toggleSit();announce('Your two-minute break is complete. Stay as long as you like.');}else if(guideDistance>=ROUTE_LENGTH){stopWalking();announce('A full circle. Stay a little, or return whenever you’re ready.');}}
  const pressed=new Set([...keys,...touchKeys]);if(!sitting&&pressed.size){if(pressed.has('turnLeft'))yaw+=dt*.8;if(pressed.has('turnRight'))yaw-=dt*.8;const forward=Number(pressed.has('forward'))-Number(pressed.has('backward')),side=Number(pressed.has('right'))-Number(pressed.has('left')),length=Math.hypot(forward,side)||1,step=saved.pace*dt;const dx=(-Math.sin(yaw)*forward+Math.cos(yaw)*side)/length*step,dz=(-Math.cos(yaw)*forward-Math.sin(yaw)*side)/length*step;const x=camera.position.x,z=camera.position.z,foot=camera.position.y-1.67;
   function support(nx,nz){const h=lookoutSupport(nx,nz,foot);if(h!==null)return h;if(lookoutZone(nx,nz)||Math.abs(groundHeight(nx,nz)-foot)>.26)return null;return canWalk(nx,nz,landscape.obstacles)?groundHeight(nx,nz):null;}
 ...
 function frame(now){
  ...
  const near=nearestRoute(camera.position.x,camera.position.z),surface=(lookoutZone(camera.position.x,camera.position.z)||spurHeight(camera.position.x,camera.position.z)!==null)?'wood':inPond(camera.position.x,camera.position.z,1.55)&&near.distance<1.65?'wood':near.distance<=1.65?(camera.position.x < -30?'grass':'gravel'):'grass';
 ...
 visit=visitControls({saved,save,announce,stopWalking,
  isGuided:()=>guided||!!towerWalk,canMove:()=>started&&!still&&!sitting&&!failed,prepareQuiet(){pebbles?.cancelAim();keys.clear();touchKeys.clear();drag=null;if(!guided)audio.resetMotion();updateControls();},
